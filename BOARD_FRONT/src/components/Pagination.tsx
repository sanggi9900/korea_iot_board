...
interface PaginationProp {
  ...
  // 특정 페이지 클릭 시 호출 함수
  handlePageClick: (page: number) => void; // 매개변수 숫자타입이며  특정작업을 수행하며 반환값은 없다

  // 이전/다음 섹션으로 이동 버튼 클릭 시 호출 함수
  handlePreSectionClick: () => void; // 매개변수도 없고 반환타입이 void 이므로 반환값도 없다
  handleNextSectionClick: () => void;
}

// 해당 pagination 메인 컴포넌트
const paginationBoxStyle = css`    // css_in_JS 적용
  display: flex;
  align-items: center;
  gap: 24px;
`;
...
const pageStyle = (isActive: boolean) => css`
  color: ${isActive ? '#1a73e8' : '#6b7280'};   // 삼항연산자
  font-size: 14px;
